...
@section('content')
    <div class="space-y-8">
        <div class="anim-fade-up" style="animation-delay: 0.05s;">
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Halaman form edit kontak</h1>
        </div>
                ...
                <div class="max-w-2xl space-y-6">
                    <div>
                        ...
                        <input type="text" id="phone" name="phone"
                            ...
                            value="{{ old('phone_number', $contacts->phone) }}" />
                        @error('phone')
                    ...
                    <div>
                        ...
                        <input type="text" id="email" name="email"
                            ...
                            placeholder="masukan email" required value="{{ old('email', $contacts->email) }}" />
                        @error('email')
                            <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="4"
                            ...
                            placeholder="Masukan alamat">{{ old('address', $contacts->address) }}</textarea>
                        @error('address')
                            <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="maps_link" name="maps_link" rows="4"
                            ...
                            placeholder="Masukan link google maps">{{ old('maps_link', $contacts->maps_link) }}</textarea>
                        @error('maps_link')
                            <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                        @enderror
                    </div>
